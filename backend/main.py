...
# ──────────────────────────────────────────────
# 프론트엔드 정적 파일 서빙 (통합 배포용)
# ──────────────────────────────────────────────

dist_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "frontend", "dist"))

# React 빌드 폴더의 assets 서빙
if os.path.exists(os.path.join(dist_dir, "assets")):
    app.mount("/assets", StaticFiles(directory=os.path.join(dist_dir, "assets")), name="assets")

# 그 외 모든 경로는 React router/index.html로 대응 (SPA 지원 및 favicon 등 처리)
@app.get("/{catchall:path}")
def read_index(catchall: str):
    # 파일이 실제로 존재하면 해당 파일 전송 (favicon.ico 등)
    ...
    return Response(
        content="Frontend build files not found. Please run 'npm run build' inside the frontend directory.",
