...
다섯 가지 카테고리로 구성. 초기에는 사전 정의된 기본 규칙으로 시작하고, **사용자 피드백을 통해 자동으로 확장·정교화**됩니다.

#### 8.A.1 [구조] Structure
- 모드 B는 양식별로 정해진 섹션 순서를 따른다
- 모드 A는 업로드된 보고서의 섹션 구조를 그대로 유지한다
- 모드 C는 다음 정형 구조를 기본 채택: 추진배경 → 현황 → 문제점 → 개선방안 → 기대효과
- 문단은 3문장 이내로 작성 (사용자 피드백 예시 — 초기 규칙으로 포함)
- 한 섹션이 너무 길면 하위 항목으로 분할

#### 8.A.2 [문체] Tone
- 종결어미: "~함", "~음", "~임" 등 명사형/격식체 위주
- 1인칭("저희가", "제가") 회피
- 능동보다 객관·중립적 표현 ("추진됨", "확인됨")
- 시제: 과거 사실은 과거형, 일반 원칙은 현재형
- 과장 표현 회피: "획기적", "최고의", "완벽한" 등

#### 8.A.3 [표현] Expression
- 도입부 관용구: "최근 ~ 따라", "~ 일환으로", "~ 관련하여"
- 현황 기술: "현재 ~ 운영 중이나", "~ 실정임"
- 문제 제기: "~ 한계 노출", "~ 우려 제기"
- 해결 제시: "~ 추진 필요", "~ 선제적 도입"
- 마무리: "~ 향상 기대", "~ 기반 마련"

#### 8.A.4 [서식] Format ★ 한글 문서 서식 규칙
보고서 본문에 적용되는 한글(HWP) 서식 규칙. 이 서식은 **빈 양식 HWPX 파일에 미리 설정**되어 본문 치환 시 자동 적용됩니다. AI 프롬프트에는 본문 작성 시 결정하는 사항(기호, 들여쓰기 등)만 명시합니다.

**서식 표준 (양식 파일에 사전 적용)**:

| 요소 | 폰트 | 크기 | 비고 |
|---|---|---|---|
| 헤드라인·목차 | HY헤드라인M | 16pt | 굵게 |
| 본문 | 휴먼명조 | 15pt | 자간 -10% 이내 |
| 하위 항목 문단 | 휴먼명조 | 15pt | 시작 기호: "ㅁ" 또는 "ㅇ" |
| 참고·각주 | 중고딕 | 13pt | 시작 기호: "*" |

**계층 구조 기호 규칙**:
- 1단계 (대분류): 숫자 + 점 (예: `1. 추진배경`)
- 2단계 (소분류): "ㅁ" 또는 "ㅇ" 기호 시작
- 3단계 (세부): "-" 또는 "·" 시작
- 참고·출처: "*" 시작, 한 줄 더 들여쓰기

**프롬프트에 들어갈 텍스트 (AI에게 지시)**:
> 본문 작성 시 다음 규칙을 따르시오:
> - 1단계 섹션은 "1. 제목" 형식으로 시작
> - 하위 항목은 "ㅁ" 또는 "ㅇ" 기호로 시작 (대분류 ㅁ, 소분류 ㅇ)
> - 추가 설명·세부 사항은 "-" 또는 "·" 기호로 시작
> - 출처·참고·각주는 "*"로 시작
> - 폰트·크기·자간은 출력 시 양식 파일이 자동 적용하므로 본문 텍스트에만 집중

#### 8.A.5 [금기] What NOT to do
- 1인칭 사용 금지
- 과장 형용사 회피
- 외래어 절제 (불가피한 경우 한글 표기 + 영문 병기)
- 불필요한 감탄·수식어 회피

#### 8.A.6 스타일 가이드의 진화

1. **초기 상태**: 위 8.A.1~8.A.5의 기본 규칙으로 시작
2. **사용자 피드백 수신**: F-08 기능으로 사용자가 자연어 피드백 입력
3. **자동 규칙 추출**: 별도 AI 호출로 피드백을 한 줄 규칙으로 변환
4. **카테고리 분류**: 위 5개 카테고리 중 하나로 분류
5. **충돌 검사**: 기존 규칙과 모순되는지 확인, 모순 시 사용자에게 확인
6. **DB 저장**: StyleRule 테이블에 추가
7. **다음 호출에 자동 반영**: 모든 후속 AI 호출의 시스템 프롬프트에 포함

→ 이 흐름을 통해 사용 시간이 길어질수록 K-water 스타일이 정교해집니다.
